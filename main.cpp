...
//    int ball_speed = 75; // стартовое значение
    ...
    while (window.isOpen()) {
        ...
        if (!pr) {
            ...
            Cannon::shoot(event, clock, pr, ball);
        }
        ...
        number_of_speed_1.change_numbers(((int) ball.get_speed()) / 10); // Смена первой цифры на экране при изменении скорости шарикка
        number_of_speed_2.change_numbers(((int) ball.get_speed()) % 10);// Смена второй цифры на экране при изменении скорости шарикка
