...
public:
    ...
    static void shoot(Event &event, Clock &clock, bool &pr, Ball &ball) {
        if (event.type == sf::Event::MouseButtonPressed) {
            if (event.mouseButton.button == sf::Mouse::Left) {
                pr = true;
                double ball_degree = ball.get_degree();
                ...
                if (!ball.speed_changed()) {
                    ball.set_speed(75);
                    ball.set_speed_start(75);
                }
                cout << "Speed in start time: " << ball.get_speed() << "\n";
                clock.restart();
            }
        }
    }
